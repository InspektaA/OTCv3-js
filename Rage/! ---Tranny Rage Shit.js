...
const utility = 
{
    ...
    console_log_prefix: "[estrogen.technology] ",
    ...
    renderer:
    {
        helpers:
        {
            ...
            render_localplayer_indicators: function()
            {
                ...
                {
                    push_indicator("DT", this.generic_color(Exploit.GetCharge()));
                }
            }
        },
        ...
        handle: function()
        {
            ...
            {
                const font = Render.AddFont("Tahoma", 8, 400);
                const string = "estrogen.technology | " + Cheat.GetUsername() + " | " + World.GetServerString();
                const string_size = Render.TextSizeCustom(string, font);
                string_size[0] += 8;
                string_size[1] += 4;
                const x = this.helpers.screen_size[0] - string_size[0] - 11, y = 80;

                Render.FilledRect(x - 4, y - 43, 3, string_size[1], [255, 255, 0, 255]);
                Render.FilledRect(x - 1, y - 43, string_size[0] + 3, string_size[1], [17, 17, 17, 255]);
                Render.StringCustom(x + 4, y - 42, 0, string, [255, 255, 255, 255], font);
            }
        }
    }
};
