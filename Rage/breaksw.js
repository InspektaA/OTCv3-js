...
function convertMatrix(rlSgrdcz) {
 return rlSgrdcz['split']('')['reduce'](function(nxVxzhnf, cf7Qrc2d) {
  if ('mqbZx' === 'mqbZx') {
   ...
   log = '[onetap] Fired shot at ' + targetName + '\x20\x28' + target + '\x29' + ' | Hitbox: ' + GetHitboxName(hitbox) + ' | Hitchance: ' + hitchance + ' | Safepoint: ' + safepoint + ' | Exploit: ' + exploit + ' | Flag: ' + info[target];
   log += '\x0a';
...
function Draw() {
 ...
 if (!UI['GetValue']('Misc', 'JAVASCRIPT', 'Script items', 'BreakSlowWalk')) return;
