...
function drawString()
{
	...
	if (isOpposite)
	{
		Render.String(screen_size[0] /2 -10, screen_size[1] /2 +20, 0, "DTap", [ 106, 120, 190, 155 ],3 );
	}
	else if (isNormal)
	{
		Render.String(screen_size[0] /2 -10, screen_size[1] /2 +20, 0, "DTap", [ 121, 99, 234, 255 ],3 );
	}
	...
	if(UI.GetValue( "Rage", "GENERAL", "Exploits", "Doubletap" ))
    {
        if(isDoubletap)
        {
			dt_color = [0 * (1.0 - Exploit.GetCharge()), 0 * Exploit.GetCharge(), 0, 0]  
        }
        else
        {
			dt_color = [255, 0, 0, 0]
	...
	if (arrows_type == 2)
	{
		...
		Render.String(screen_size[0] /2, screen_size[1] /2 +1000, 1,  ".", drawBack ? s_arrow_color : arrows_color, 4 );		
	}

	if(drawLeft)
	{
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +49, 0, isInverted ? "LEFT" : "RIGHT", [ 209, 139, 230, 0 ],3 );
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +58, 0, is_DT ? DT : DT, dt_color,3 );
		if (isHIDESHOTS) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "ONSHOT", [ 0, 195, 13, 255 ],3 );
		}
		if (isDMG) {
			add_y = add_y + 9;
		    Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "DMG", [ 255, 101, 0, 255 ],3 );
		}
		if (isFD) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "DUCK", [ 255, 255, 255, 0 ] ,3 );
		}
		if (isSP) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "SP", [ 124, 195, 13, 0 ] ,3 );
		}
		if (isOVHBX) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "FB", [ 124, 195, 13, 0 ] ,3 );
		}
	}
	else if(drawRight)
	{	
	...
	else if(drawBack)
	{
		...
		if (isHIDESHOTS) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "ONSHOT", [ 124, 195, 13, 0 ],3 );
		}
		if (isDMG) {
			add_y = add_y + 9;
		    Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, -10, "DMG", [ 255, 101, 0, 255 ],3 );
		}
		if (isFD) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "DUCK", [ 255, 255, 255, 0 ],3 );
	...
	{
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +0, 0, isHideReal ? "HIDE" : "HIDE", [ 209, 139, 230, 0 ],3 );
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +58, 0, is_DT ? DT : DT, dt_color,3 );
		if (isHIDESHOTS) {
			add_y = add_y + -18;
			Render.String(screen_size[0] /2 -15, screen_size[1] /2 + add_y, 0, "ONSHOT", [ 186, 179, 234, 200 ],3 );
		}
		if (isDMG) {
			add_y = add_y + -10;
		    Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, +5, "DAMAGE", [ 255, 101, 0, 200 ],3 );
		}
		if (isFD) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2+5 , screen_size[1] /2 + add_y, 0, "DUCK", [ 255, 255, 255, 0 ],3 );
